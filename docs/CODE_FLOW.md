...
|------|---------|---|
| `src/api/main.py` | HTTP API entry | `/chat/search`, `/session/{id}` |
...
| `src/search/graph.py` | Neo4j relationship queries | `GraphSearcher` (Phase 5) |
...
```
**Files involved:** `session/manager.py`, `models/api.py`  
...
state: GraphState = {
    ...
    "merged_results": [],    # Previous results
    ...
}
...
```
**Files involved:** `langgraph/graph.py`, `langgraph/nodes.py`  
**Outcome:** Fully populated state with answer + sources

---

## Pipeline Nodes (10 Core + 2 Optional)
