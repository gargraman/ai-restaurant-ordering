...
@pytest.mark.asyncio
async def test_get_cart_success(client, db_session):
    ...
    with patch('src.api.routers.orders.CartManager') as mock_cart_manager_class:
        ...
        mock_cart.restaurant_id = str(uuid4())
        ...
        # Act
        response = client.get(f"/orders/cart?session_id={session_id}&tenant_id={tenant_id}")

        # Assert
        assert response.status_code == 200
        data = response.json()
        assert data["session_id"] == session_id
        assert data["restaurant_id"] == mock_cart.restaurant_id
...
@pytest.mark.asyncio
async def test_add_to_cart_success(client, db_session):
    ...
    with patch('src.api.routers.orders.CartManager') as mock_cart_manager_class:
        ...
        mock_item.menu_item_id = str(uuid4())
        mock_item.name = "Test Item"
        mock_item.quantity = 1
        mock_item.unit_price_cents = 1000
        ...
        assert data["subtotal_cents"] == 1000


@pytest.mark.asyncio
async def test_update_cart_item_success(client, db_session):
    ...
    with patch('src.api.routers.orders.CartManager') as mock_cart_manager_class:
        ...
        # Act
        response = client.patch(f"/orders/cart/items/{item_id}?session_id={session_id}&tenant_id={tenant_id}",
                               json=update_data)
...
@pytest.mark.asyncio
async def test_remove_cart_item_success(client, db_session):
    ...
    with patch('src.api.routers.orders.CartManager') as mock_cart_manager_class:
        ...
        # Act
        response = client.delete(f"/orders/cart/items/{item_id}?session_id={session_id}&tenant_id={tenant_id}")
...
@pytest.mark.asyncio
async def test_clear_cart_success(client, db_session):
    ...
    with patch('src.api.routers.orders.CartManager') as mock_cart_manager_class:
        ...
        # Act
        response = client.delete(f"/orders/cart?session_id={session_id}&tenant_id={tenant_id}")
...
async def test_create_order_success(client, db_session):
    ...
    with patch('src.api.routers.orders.OrderService') as mock_order_service_class, \
        ...
        mock_order.restaurant_id = str(uuid4())
        ...
        mock_payment_intent.stripe_payment_intent_id = f"pi_{uuid4().hex}"
        ...
        mock_order_service.clear_cart_after_order.return_value = None
...
@pytest.mark.asyncio
async def test_get_order_success(client, db_session):
...
@pytest.mark.asyncio
async def test_list_orders_success(client, db_session):
