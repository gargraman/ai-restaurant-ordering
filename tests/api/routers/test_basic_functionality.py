...
def test_webhooks_endpoints_exist(client):
    ...
    # Test stripe webhook - will likely return 400 for missing signature or 404 if not configured
    response = client.post("/webhooks/stripe", content="{}")
    assert response.status_code in [400, 422, 404]  # 400 for missing signature, 404 if webhook not configured
