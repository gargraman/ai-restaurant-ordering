...
class TestNotifyOrderFailure:
    ...
    async def test_notify_order_failure_success(self):
        ...
        with patch('src.db.base.AsyncSessionLocal') as mock_session_local, \
            ...
            assert result["order_id"] == "order-123"
            assert len(result["notifications_sent"]) == 3  # customer_email, customer_sms, restaurant
...
class TestRetryDLQOrder:
    ...
    async def test_retry_dlq_order_success(self):
        ...
        mock_order = MagicMock()
        mock_order.status.value = "failed"
        mock_order.status = MagicMock()
        mock_order.status.name = "FAILED"
        ...
        with patch('src.db.base.AsyncSessionLocal') as mock_session_local, \
            ...
            assert mock_order.status.value == "paid"  # Status should be reset to paid
...
class TestNotificationHelpers:
    ...
    async def test_send_customer_email(self):
        ...
        with patch('src.notifications.sendgrid_service.send_customer_notification_email') as mock_send:
    ...
    async def test_send_customer_sms(self):
        ...
        with patch('src.notifications.sendgrid_service.send_customer_sms_notification') as mock_send:
            mock_send.return_value = {"status": "sent"}
